...
$window.onscroll=function(){
     ...
     if(scrollTop + windowHeight >= scrollHeight){
		workPlan.pages++;
		$http.post('http://'+url+'/oa/App/AppProject!waitWorkList.action?pages='+workPlan.pages+"&userId="+workPlan.user.id).success(function(data){
...
result = false;
    workPlan.pages=1;
request.post('http://'+url+'/oa/App/AppProject!waitWorkList.action?pages='+"1"+"&userId="+model.user.id).success(function(data){
