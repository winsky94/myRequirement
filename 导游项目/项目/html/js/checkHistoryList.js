...
$window.onscroll=function(){
      ...
      if(scrollTop + windowHeight >= scrollHeight){
		apply.pages++;
		$http.post('http://'+url+'/oa/App/Apply!historyList.action?pages='+apply.pages+"&userId="+apply.user.id).success(function(data){
...
function LoadData(){
	...
	if(!first) creatMask();
    apply.pages=1;
request.post('http://'+url+'/oa/App/Apply!historyList.action?pages='+"1"+"&userId="+model.user.id).success(function(data){
