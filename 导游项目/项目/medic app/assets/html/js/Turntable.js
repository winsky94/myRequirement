...
Turntable = function(d) {
    ...
    this.mFont   = d.font  || "20px sans-serif";
    // 加载图片
    this.mBackground=new  Image();
    this.mBackground.src = d.background;
    this.mForeground=new  Image();
    this.mForeground.src = d.foreground;
    this.mP=new Image();
    this.mP.src = d.p;
    // 运动参数
    this.mAngle = 0;
...
Turntable.prototype.draw =function() {
    ...
    this.mContext.drawImage(this.mBackground, 0,0,this.mBackground.width,this.mBackground.height, 0,0,2*this.mRadius,2*this.mRadius);
    // 绘制转盘
    this.mContext.save();
    ...
    this.mContext.rotate(this.mAngle);
    {
        ...
        for (var i = 0; i < this.mDiv; i++) {
            ...
            // 绘制栏目
            {
                var cw= (this.mRadius-this.mPadding)* dg  /2 ;
                var ch=cw  /this.mForeground.width*this.mForeground.height;
                this.mContext.save();
                this.mContext.rotate(dg/2);
                this.mContext.translate(0 ,-3*this.mRadius/4);
                this.mContext.drawImage(this.mForeground,0,0 ,this.mForeground.width,this.mForeground.height, -cw/2,-ch/4,cw, ch);
                ...
                this.mContext.fillText(this.mItems[i], 0,ch/4);
